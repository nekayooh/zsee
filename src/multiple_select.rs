...
#[derive(Props)]
pub struct ZMultipleSelectProps<'a> {
...
fn get_value_from_key(option:Vec<Vec<String>>,keys:Vec<String>)-> String{
    let mut vecs = vec![];
    for key in  keys.iter() {
        let one:Vec<Vec<String>> = option.iter().filter(|one|one[0].clone()==key.clone()).cloned().collect();
        if one.len()>0 {
            vecs.push(one[0][1].clone())
        }
    }
    vecs.join(",")
...
pub fn ZMultipleSelect<'a>(cx: Scope<'a, ZMultipleSelectProps<'a>>) -> Element {
    // 是否显示下拉列表
                ...
                div {
                    ...
                    cx.props.option.iter().filter(|each| each[1].contains(psearch.get())).map(|each|{
                        cx.render(rsx!(div{
                            ...
                            zab!( "bg-".to_owned()+zshare.read().color.as_str()+"-100 text-"+zshare.read().color.as_str()+"-500",cx.props.color==""&&key.get().contains(&each[0])),
                            zab!( "bg-".to_owned()+cx.props.color.as_str()+"-100 text-"+cx.props.color.as_str()+"-500",cx.props.color!=""&&key.get().contains(&each[0]))),
                            onclick:move |_| {
                                // 赋值
                                
                                let mut keys:Vec<String> = vec![];
                                if key.get().contains(&each[0]) {
                                    for k in key.get().clone().iter(){
                                        if k.clone()!=each[0] {
                                            keys.push(k.clone());
                                        }
                                    }
                                    ...
                                    keys =  key.get().clone();
                                    keys.push(each[0].to_string());
                                }
                                search.set(get_value_from_key(cx.props.option.clone(), keys.clone()));
                                key.set(keys.clone());
                                cx.props.onchange.call(keys);
                            },
                            if key.get().contains(&each[0]) {
                                cx.render(rsx!(ZIcon {
                                    icon:"check",
